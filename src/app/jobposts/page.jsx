...
// // State to check if there are more posts to load
// const [hasMore, setHasMore] = useState(true);

// // Reference to keep track of the Intersection Observer instance
...
//     try {
//       setLoading(true)
//       await axios.get('http://localhost:3001//post/getjobs', {
//         params: { page, limit: 15 },
//       })
//         .then((res) => {
//            // Append the newly fetched posts to the existing list of posts
//       setJobs((prevJobs) => [...prevJobs, ...res.data.jobs]);
//           console.log(res.data.jobs)
//            // Update the `hasMore` flag to determine if more posts are available
...
//   // Callback for the Intersection Observer, which triggers when the last post element is in view
//   const lastPostRef = useCallback(
//     (node) => {
//       // Disconnect the existing observer to avoid duplicating observers
//       if (observer.current) observer.current.disconnect();

//       // Create a new Intersection Observer
//       observer.current = new IntersectionObserver((entries) => {
//         // If the last post is visible and there are more posts to load, increment the page
//         if (entries[0].isIntersecting && hasMore) {
//           setPage((prevPage) => prevPage + 1);
//         }
//       });

//       // If a node is found (last post element), start observing it
//       if (node) observer.current.observe(node);
//     },
//     [hasMore] // Dependency array to recreate the observer when `hasMore` changes
//   );
...
//              // Attach `lastPostRef` to the last post element to detect when it comes into view
          
//             <motion.div
//               key={job.id}
//               ref={index === filteredJobs.length - 1 ? lastPostRef : null} // Attach `lastPostRef` to the last post element to detect when it comes into view
...
//           ))}
//         </AnimatePresence>
//         {loading && <JobSearchLoading />} {/* Display loading indicator */}
//       </div>)}
